export default class Time extends Date {
    constructor(value, monthIndex, date, hours, minutes, seconds, ms) {
     ...
     */
    getWeek() {
        return Math.ceil(Math.ceil((this.valueOf() -
            new Time(this.getFullYear(), 0, 9 - (this.format('yyyy-01-01', true).getDay() || 7)).valueOf()) /
            86400000) / 7);
    }
    format(format, toDate) {
        ...
        [/y+/, /M+/, /d+/, /h+/, /H+/, /m+/, /s+/, /q+/, /w+/, /W+/].forEach((regexp, i) => {
            (format.match(regexp) || []).forEach((matchStr) => {
                let replace = '';
                if (i === 0) {
                ...
                if (i < 7) {
                    format = format.replace(matchStr, matchStr.length === 1 ? replace : replace.padStart(matchStr.length, '0'));
                }
                else if (i === 7) {
                    format = format.replace(matchStr, matchStr.length >= 4 ? `第${replace}季度` : replace);
                }
                else if (i === 8) {
                    format = format.replace(matchStr, matchStr.length > 1
                        ? matchStr.length > 2
                            ? `星期${replace}`
                            : `周${replace}`
                        : replace);
    ...
    getDifference(date, format) {
        ...
        [/d+/, /h+/, /m+/, /s+/].forEach((regexp, i) => {
            (format.match(regexp) || []).forEach((matchStr) => {
                ...
                differenceStr = differenceStr.replace(matchStr, matchStr.length === 1 ? replace : replace.padStart(matchStr.length, '0'));
            });
        });
